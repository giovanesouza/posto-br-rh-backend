...
describe("User Routes", () => {
  it("1 should create a user successfully", async () => {
  ...
  it("2 should fail when username is missing", async () => {
    const res = await request(app)
      .post("/users")
      .set("Authorization", `Bearer ${global.token}`)
      .send({
  ...
  it("3 should fail when multiple fields are missing", async () => {
    const res = await request(app)
      .post("/users")
      .set("Authorization", `Bearer ${global.token}`)
      .send({});
  ...
  it("4 should list all users", async () => {
    const res = await request(app)
      .get("/users")
      .set("Authorization", `Bearer ${global.token}`);
  ...
  it("5 should return user by id", async () => {
    const res = await request(app)
      .get(`/users/${validUUID}`)
      .set("Authorization", `Bearer ${global.token}`);
  ...
  it("7 should update a user successfully", async () => {
    const res = await request(app)
